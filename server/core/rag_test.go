...
func TestIndexAsync(t *testing.T) {
	...
	for _, m := range msg {
		...
		if m.MetaData[common.FieldExtra] != nil {
			if err = json.Unmarshal([]byte(m.MetaData[common.FieldExtra].(string)), &extData); err != nil {
				t.Fatal(err)
			}
			extData[common.FieldQAContent] = ""
		}
		item := &schema.Document{
			Content:  "===================== 这个文档被我修改了 =====================\n" + m.Content,
			ID:       m.ID,
			MetaData: extData, // 修改后的 MetaData
		}
